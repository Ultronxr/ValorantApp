...
public interface RiotAccountMapper extends BaseMapper<RiotAccount> {
    ...
    /**
     * 计数 - 预更新了 access token 的拳头账号数量（且账号未被删除、登录验证成功的、当天没有更新每日商店数据的）<br/>
    ...
    Long selectCountForPreUpdatedTokenAccount(@Param("date") final Date now);

    /**
     * 计数 - 未预更新 access token 的拳头账号数量（且账号未被删除、登录验证成功的、当天没有更新每日商店数据的）<br/>
    ...
    Long selectCountForNotPreUpdatedTokenAccount(@Param("date") final Date now);

    /**
     * 获取 ID List - 预更新了 access token 的拳头账号 ID （且账号未被删除、登录验证成功的、当天没有更新每日商店数据的）<br/>
     * “预更新了”的成立条件：token 过期时间 > 当前时间
     *
     * @param now           当前时间
    ...
    List<String> selectIdListForPreUpdatedTokenAccount(@Param("date") final Date now,
                                                       @Param("isDistributed") final boolean isDistributed,
                                                       @Param("sqlLimitIndex") final long sqlLimitIndex,
                                                       @Param("sqlLimitCount") final long sqlLimitCount);

    /**
     * 获取 ID List - 未预更新 access token 的拳头账号 ID （且账号未被删除、登录验证成功的、当天没有更新每日商店数据的）<br/>
     * “未预更新”的成立条件：token 过期时间 <= 当前时间 || token 过期时间为空
     *
     * @param now           当前时间
     * @param isDistributed 是否分布式（是分布式，则使用 SQL LIMIT 只获取部分数据；不是分布式，则直接获取全量数据）
    ...
    List<String> selectIdListForNotPreUpdatedTokenAccount(@Param("date") final Date now,
                                                          @Param("isDistributed") final boolean isDistributed,
                                                          @Param("sqlLimitIndex") final long sqlLimitIndex,
                                                          @Param("sqlLimitCount") final long sqlLimitCount);
